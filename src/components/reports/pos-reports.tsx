...
export default function PosReports({ agencyId, user, dateRange }: PosReportsProps) {
  ...
  const exportPosReport = async (format: string) => {
    try {
      setIsExporting(true)
      const result = await exportReportData(agencyId, "pos", format, dateRange, {
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                ...
                <DropdownMenuItem onClick={() => exportPosReport("WORD")}>📝 Exportar Word</DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportPosReport("PNG")}>🖼️ Exportar PNG</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </CardContent>
