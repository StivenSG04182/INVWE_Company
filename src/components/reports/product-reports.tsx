...
import { ArrowDownToLine, Calendar, TrendingUp, Package, Loader2, Download } from "lucide-react"
...
export default function ProductReports({
  ...
  const exportProducts = async (format: string) => {
    try {
      setIsExporting(true)
      const result = await exportReportData(agencyId, "inventory", format, dateRange, {
        category: "all",
        search: "",
      })
      ...
      if (result && result.fileBase64 && result.fileName && result.mimeType) {
        ...
        toast({
          ...
          description: `Reporte de productos exportado en formato ${format}`,
        })
      } else {
        ...
        <CardContent>
          <div className="flex flex-wrap gap-3">
            <DropdownMenu>
              ...
              <DropdownMenuContent>
                <DropdownMenuItem onClick={() => exportProducts("PDF")}>📄 Exportar PDF</DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportProducts("EXCEL")}>📊 Exportar Excel</DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportProducts("WORD")}>📝 Exportar Word</DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportProducts("CSV")}>📋 Exportar CSV</DropdownMenuItem>
                <DropdownMenuItem onClick={() => exportProducts("PNG")}>🖼️ Exportar PNG</DropdownMenuItem>
              </DropdownMenuContent>
            </DropdownMenu>
          </div>
        </CardContent>
