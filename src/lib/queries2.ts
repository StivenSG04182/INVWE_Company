    ...
    for (const product of products) {
        ...
        for (const movement of productMovements) {
            ...
            if (movement.type === 'transferencia' && movement.destinationAreaId) {
                if (!areaMovements[movement.destinationAreaId]) {
                    areaMovements[movement.destinationAreaId] = [];
                }
        ...
        for (const areaId in areaMovements) {
            ...
            for (const movement of areaMovements[areaId]) {
                ...
                } else if (movement.type === 'salida' && movement.areaId === areaId) {
                    quantity -= movement.quantity;
                } else if (movement.type === 'transferencia') {
                    if (movement.areaId === areaId) {
                        // Salida del área de origen
                        quantity -= movement.quantity;
                    }
                    if (movement.destinationAreaId === areaId) {
                        // Entrada al área de destino
                        quantity += movement.quantity;
                    }
                }
            }
            ...
            if (quantity > 0 || product.minStock) {
                stocks.push({
                    id: `${product.id}-${areaId}`,
                    _id: `${product.id}-${areaId}`,
