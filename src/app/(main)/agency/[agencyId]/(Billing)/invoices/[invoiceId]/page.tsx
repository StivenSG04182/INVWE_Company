...
const InvoiceDetailsPage = async ({ params }: { params: { agencyId: string; invoiceId: string } }) => {
    ...
    if (!invoice) {
        return redirect(`/agency/${agencyId}/invoices`);
    }
    ...
    return (
        <div className="flex flex-col gap-6 p-4">
            <div className="flex justify-between items-center">
                <Link href={`/agency/${agencyId}/invoices`}>
                ...
                <CardHeader className="flex flex-row justify-between items-start">
                    <div>
                        ...
                        <CardDescription>
                            ...
                            {invoice.dueDate && (
                                <> · Vence el {new Date(invoice.dueDate).toLocaleDateString()}</>
                            )}
                        </CardDescription>
                    </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="space-y-1">
                                ...
                                <div className="flex justify-between">
                                    ...
                                    <span>{invoice.status === 'DRAFT' ? 'Borrador' :
                                        invoice.status === 'PENDING' ? 'Pendiente' :
                                            invoice.status === 'PAID' ? 'Pagada' :
                                                invoice.status === 'CANCELLED' ? 'Cancelada' : 'Vencida'}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                <tbody>
                                    {invoice.Items.map((item) => (
                                        <tr key={item.id} className="border-b">
                                            ...
                                            <td className="px-4 py-2 text-right">{Number(item.discount) > 0 ? `${item.discount}%` : '-'}</td>
