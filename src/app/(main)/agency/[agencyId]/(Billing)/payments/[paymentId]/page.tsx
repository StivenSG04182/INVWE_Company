...
const PaymentDetailsPage = async ({ params }: { params: { agencyId: string; paymentId: string } }) => {
    ...
    if (!payment) {
        return redirect(`/agency/${agencyId}/payments`);
    }

    return (
        <div className="flex flex-col gap-6 p-4">
            <div className="flex justify-between items-center">
                <Link href={`/agency/${agencyId}/payments`}>
                ...
                <div className="flex gap-2">
                    ...
                    {payment.Invoice && (
                        <Link href={`/agency/${agencyId}/invoices/${payment.Invoice.id}`}>
                                                ...
                                                payment.method === 'DEBIT_CARD' ? 'Tarjeta de Débito' :
                                                    payment.method === 'BANK_TRANSFER' ? 'Transferencia Bancaria' :
                                                        payment.method === 'CHECK' ? 'Cheque' :
                                                            payment.method === 'ONLINE' ? 'Pago en Línea' : 'Otro'}
                                    </span>
                                </div>
                                <div className="flex justify-between">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <div className="flex justify-between">
                                        ...
                                        <span>
                                            {payment.Invoice.status === 'DRAFT' ? 'Borrador' :
                                                payment.Invoice.status === 'PENDING' ? 'Pendiente' :
                                                    payment.Invoice.status === 'PAID' ? 'Pagada' :
                                                        payment.Invoice.status === 'CANCELLED' ? 'Cancelada' : 'Vencida'}
                                        </span>
                                    </div>
                                    <div className="flex justify-between">
