...
const FunnelSteps = ({ funnel, funnelId, pages, subaccountId }: Props) => {
  ...
  const onDragEnd = (dropResult: DropResult) => {
    ...
    newPageOrder.forEach(async (page, index) => {
      try {
        ...
        toast({
          variant: 'destructive',
          title: 'Fallido',
          description: 'No se ha podido guardar el orden de la página',
        })
        return
      }
    })
    ...
    toast({
      title: 'Éxito',
      description: 'Orden de páginas guardado',
    })
  }
        ...
        <aside className="flex-[0.3] bg-background p-6  flex flex-col justify-between ">
          ...
          <Button
            ...
            onClick={() => {
              setOpen(
                <CustomModal
                  title="Crear o actualizar una página de comercio electrónico"
                  subheading="Las páginas de comercio electrónico le permiten crear procesos paso a paso para que los clientes los sigan."
                >
                  <CreateFunnelPage
        ...
        <aside className="flex-[0.7] bg-muted p-4 ">
          {!!pages.length ? (
            <Card className="h-full flex justify-between flex-col">
              <CardHeader>
                <p className="text-sm text-muted-foreground">Nombre de la página</p>
                <CardTitle>{clickedPage?.name}</CardTitle>
                <CardDescription className="flex flex-col gap-4">
                  <div className="border-2 rounded-lg sm:w-80 w-full  overflow-clip">
                    <Link
                      href={`/agency/${subaccountId}/funnels/${funnelId}/editor/${clickedPage?.id}`}
            ...
            <div className="h-[600px] flex items-center justify-center text-muted-foreground">
              Cree una página para ver la configuración de la página.
            </div>
          )}
        </aside>
      </div>
